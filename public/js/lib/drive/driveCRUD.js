...
function createProject(folderId,title,callback){
	...
	gapi.client.drive.files.insert({'resource': data}).execute(function(resp){
		if (!resp.error){
			insertFileIntoFolder(folderId,resp.id,function(resp2){
				if (!resp2.error){
					console.log(successMessage,':',title,resp2);
					
					// remove the parent(root in google drive) of the folder (project)
					// make sure it is not root
					if (folderId != 'root') {
						removeFileFromFolder('root', resp2.id,function(resp3){
							//callback
							if (!resp3.error){
								console.log('remove a file from a folder succeed');
								callback && callback(successMessage,true,resp2);
							}
						})							
					}else{
						callback && callback(successMessage,true,resp2);
...
//			console.log("Result:"+JSON.stringify(result[0]));
			...
			if (nextPageToken) {
				...
				//wait until all the files information are obtained
				console.log("retrieveChildrenFiles:", result);
...
function makeFileObject(oldFile){
	...
	, id = oldFile.id
	, parent = oldFile.parents
	, mimeType = oldFile.mimeType
	, permission = oldFile.userPermission
	, header = oldFile.header
	, content = oldFile.content
	, parents = oldFile.parents
	, iconLink = oldFile.iconLink
	var file = {title:title,id:id,createdDate:createdDate,modifiedDate:modifiedDate,unformattedModifiedDate:unformattedModifiedDate,link:link,mimeType:mimeType,permission:permission, header:header, content:content, parents:parents, iconLink:iconLink};
...
/*
 * Remove a parent(folder) of a file
 */
 ...
 */
function uploadFileFromDropbox(folderId,fileName,dataBlob,callback) {
	var successMessage = "Upload from dropbox complete.";
	var failMessage = "Sorry, upload from dropbox incomplete";
	//first insert the file into drive
	insertFile(dataBlob,function(resp){
		if (!resp.error){
			//then move the file into destinated folder, e.g. LabBook folder
			insertFileIntoFolder(folderId,resp.id,function(resp2){
				if (!resp2.error){
					...
					//remove from the old parent, i.e. root
					removeFileFromFolder('root', resp2.id,function(resp3){
						//callback
						if (!resp3.error){
							console.log('remove a file from a folder succeed');
							
							//always the last name in the path
							neatFileName = fileName.split('/');
							neatFileName = neatFileName[neatFileName.length-1];
							
							renameFile(resp2.id,neatFileName,function(resp4){
								if (!resp4.error){
									console.log('rename succeed');
								}
								
								callback && callback(successMessage,true,resp4);
							});							
						}						
					});
				}else{
